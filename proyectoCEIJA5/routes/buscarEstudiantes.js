...
// GET /api/buscar-estudiantes
// Query params: q (texto), page, limit, activo (0/1), modalidadId, estadoId
router.get('/', async (req, res) => {
  try {
    ...
    // Build base conditions (without activo) so we can compute activos/inactivos counts separately
    const baseConds = [];
    const baseParams = [];

    // Search text: match nombre OR apellido OR dni
    const qTrim = String(q || '').trim();
    if (qTrim) {
      baseConds.push('(e.nombre LIKE ? OR e.apellido LIKE ? OR e.dni LIKE ?)');
      const like = `%${qTrim}%`;
    ...
    // Total counts (without activo filter) for footer
    ...
    // For the listing itself, apply filtro activo default to 1 (like other endpoints) unless activo param is provided
    ...
    // Fetch full student rows for those ids
    const placeholders = ids.map(() => '?').join(',');
    // If modality or estado should filter the joined inscripciones, add ON conditions
