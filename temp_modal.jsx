...
const ModalRegistrosPendientes = ({ onClose }) => {
    ...
    const handleRegistroGuardado = async (registro, tipoOperacion) => {
        ...
        if (tipoOperacion === 'completado') {
            // Si se completó, mostrar mensaje de éxito informando que está registrado y Completa
            const nombreCompleto = `${registro.datos?.nombre || registro.nombre} ${registro.datos?.apellido || registro.apellido}`;
            showSuccess(`🎉 ${nombreCompleto} - Estudiante registrado y Completa. Puede eliminarlo del listado con el botón Eliminar.`);
    ...
    const procesarEliminacion = async (registro, nombreCompleto) => {
        try {
            setMensajeEmail(`🗑️ Eliminando ${nombreCompleto} del listado de pendientes...`);
            ...
            setMensajeEmail(`✅ ${nombreCompleto} eliminado del listado de pendientes`);
            setTimeout(() => setMensajeEmail(''), 3000);

            showSuccess(`🗑️ ${nombreCompleto} eliminado del listado de pendientes. Si ya está registrado y Completa, el proceso está completo.`);

        } catch (error) {
            console.error('❌ Error al eliminar registro:', error);
            setMensajeEmail(`❌ Error al eliminar: ${error.message}`);
            setTimeout(() => setMensajeEmail(''), 5000);
